...
static void usb_findcart()
{
    ...
    // Read the cartridge and check if we have a 64Drive.
        ...
        #elif USE_EPI
            osEPiReadIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_MAGIC, &buff);
        #else
    ...
    // Read the cartridge and check if we have a SummerCart64.
        ...
        #elif USE_EPI
            osEPiReadIo(CART_HANDLE, SC64_REG_VERSION, &buff);
        #else
...
static s8 usb_64drive_wait()
{
    ...
    do
    {
            ...
            #elif USE_EPI
                osEPiReadIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_STATUS, &ret);
            #else
...
static void usb_64drive_setwritable(u8 enable)
{
    usb_64drive_wait();
        ...
        #elif USE_EPI
            osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_COMMAND, enable ? D64_ENABLE_ROMWR : D64_DISABLE_ROMWR);
        #else
...
static int usb_64drive_waitidle()
{
    ...
    do 
    {
            ...
            #elif USE_EPI
                osEPiReadIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBCOMSTAT, &status);
            #else
...
static void usb_64drive_write(int datatype, const void* data, int size)
{
    ...
    while (left > 0)
    {
        ...
        // Set up DMA transfer between RDRAM and the PI
            ...
            osWritebackDCache(usb_buffer, block);
                ...
                dmaIOMessageBuf.dramAddr = usb_buffer;
                dmaIOMessageBuf.devAddr = D64_BASE_ADDRESS + DEBUG_ADDRESS + read;
                dmaIOMessageBuf.size = block;
                osEPiStartDma(CART_HANDLE, &dmaIOMessageBuf, OS_WRITE);
    ...
    // Send the data through USB
        ...
        #elif USE_EPI
            osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBP0R0, (DEBUG_ADDRESS) >> 1);
            osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBP1R1, (size & 0xFFFFFF) | (datatype << 24));
            osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBCOMSTAT, D64_COMMAND_WRITE);
        #else
...
static void usb_64drive_arm(u32 offset, u32 size)
{
    ...
    if (ret != D64_USB_ARMING && ret != D64_USB_ARMED)
    {
        ...
        // Arm the 64Drive, using the ROM space as a buffer
            ...
            #elif USE_EPI
                osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBCOMSTAT, D64_USB_ARM);
                osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBP0R0, (offset >> 1));
                osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBP1R1, (size & 0xFFFFFF));
            #else
...
static void usb_64drive_disarm()
{
    // Disarm the USB
        ...
        #elif USE_EPI
            osEPiWriteIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBCOMSTAT, D64_USB_DISARM); 
        #else
...
static u32 usb_64drive_poll()
{
    ...
    if (usb_64drive_armstatus() == D64_USB_DATA)
    {
        // Read the data header from the Param0 register
            ...
            #elif USE_EPI
                osEPiReadIo(CART_HANDLE, D64_CIBASE_ADDRESS + D64_REGISTER_USBP0R0, &ret);
            #else
...
static void usb_64drive_read()
{
    // Set up DMA transfer between RDRAM and the PI
        ...
        osWritebackDCacheAll();
            ...
            dmaIOMessageBuf.hdr.retQueue = &dmaMessageQ;
            dmaIOMessageBuf.dramAddr = usb_buffer;
            dmaIOMessageBuf.devAddr = D64_BASE_ADDRESS + DEBUG_ADDRESS + usb_readblock;
            dmaIOMessageBuf.size = BUFFER_SIZE;
            osEPiStartDma(CART_HANDLE, &dmaIOMessageBuf, OS_READ);
            (void)osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
        #else
    ...
    // Set up DMA transfer between RDRAM and the PI
        ...
        *(volatile unsigned long *)(N64_PI_ADDRESS + N64_PI_WRITELENGTH) = len-1;
        ...
        osInvalDCache(buff, len);
            ...
            dmaIOMessageBuf.hdr.pri = OS_MESG_PRI_NORMAL;
            dmaIOMessageBuf.hdr.retQueue = &dmaMessageQ;
            dmaIOMessageBuf.dramAddr = buff;
            dmaIOMessageBuf.devAddr = pi_address;
    ...
    // Set up DMA transfer between RDRAM and the PI
        ...
        *(vu32*)0xA4600010 = 3;
        ...
        osWritebackDCacheAll();
            ...
            dmaIOMessageBuf.hdr.retQueue = &dmaMessageQ;
            dmaIOMessageBuf.dramAddr = usb_buffer;
            dmaIOMessageBuf.devAddr = ED_BASE + DEBUG_ADDRESS + usb_readblock;
            dmaIOMessageBuf.size = BUFFER_SIZE;
            osEPiStartDma(CART_HANDLE, &dmaIOMessageBuf, OS_READ);
            (void)osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
        #else
...
static u32 usb_sc64_read_usb_scr(void)
{
    u32 usb_scr __attribute__((aligned(8)));
        ...
        #elif USE_EPI
            osEPiReadIo(CART_HANDLE, SC64_REG_USB_SCR, &usb_scr);
        #else
...
static u32 usb_sc64_read_usb_fifo(void)
{
    u32 data __attribute__((aligned(8)));
        ...
        #elif USE_EPI
            osEPiReadIo(CART_HANDLE, SC64_MEM_USB_FIFO_BASE, &data);
        #else
...
static void usb_sc64_setwritable(u8 enable)
{
    u32 scr __attribute__((aligned(8)));
        ...
        #elif USE_EPI
            osEPiReadIo(CART_HANDLE, SC64_REG_SCR, &scr);
            osEPiWriteIo(CART_HANDLE, SC64_REG_SCR, enable ? (scr | SC64_SCR_SDRAM_WRITE_EN) : (scr & (~SC64_SCR_SDRAM_WRITE_EN)));
        #else
    ...
    while (left > 0)
    {
        ...
        // Write data to buffer in SDRAM
            ...
            osWritebackDCache(usb_buffer, dma_length);
                ...
                dmaIOMessageBuf.hdr.retQueue = &dmaMessageQ;
                dmaIOMessageBuf.dramAddr = usb_buffer;
                dmaIOMessageBuf.devAddr = sdram_address;
                dmaIOMessageBuf.size = dma_length;
                osEPiStartDma(CART_HANDLE, &dmaIOMessageBuf, OS_WRITE);
        ...
        // Setup hardware registers
            ...
            #elif USE_EPI
                osEPiWriteIo(CART_HANDLE, SC64_REG_USB_DMA_ADDR, SC64_USB_BANK_ADDR(SC64_BANK_ROM, DEBUG_ADDRESS));
                osEPiWriteIo(CART_HANDLE, SC64_REG_USB_DMA_LEN, SC64_USB_LENGTH(transfer_length));
                osEPiWriteIo(CART_HANDLE, SC64_REG_USB_SCR, SC64_USB_CONTROL_START);
            #else
        ...
        while (left > 0)
        {
            ...
            // Load data from FIFO to buffer in RDRAM
                ...
                dma_write(usb_buffer, sdram_address, dma_length);
                    ...
                    dmaIOMessageBuf.hdr.retQueue = &dmaMessageQ;
                    dmaIOMessageBuf.dramAddr = usb_buffer;
                    dmaIOMessageBuf.devAddr = SC64_MEM_USB_FIFO_BASE;
                    dmaIOMessageBuf.size = dma_length;
                    osEPiStartDma(CART_HANDLE, &dmaIOMessageBuf, OS_READ);
                    osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
                #else
